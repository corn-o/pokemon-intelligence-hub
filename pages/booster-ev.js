...
export default function BoosterEVPage() {
  ...
  async function calculateBoosterEV(e) {
    ...
    try {
      ...
      setEvError(err.message || 'Failed to calculate EV. Try again later.')
    } finally {
      setEvLoading(false)
    }
  }
  ...
  return (
    <div>
      <h1>📦 Booster EV Calculator</h1>
      <p>
        Enter a modern set name to estimate booster-box expected value and
        inspect release-to-now market snapshots.
      </p>

      <form
        onSubmit={calculateBoosterEV}
        style={{ display: 'grid', gap: '0.75rem', maxWidth: '560px' }}
      >
        <label>
          Set
          <input
